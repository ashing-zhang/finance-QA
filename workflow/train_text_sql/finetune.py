...
def train(args):
    ...
    while cur_step < total_steps:
        ...
        # inputs["lables"].device: cuda:0
        # inputs["input_features"].device: cuda:0
        # inputs["feature_attention_mask"].device: cuda:0
        # inputs["attention_mask"].device: cuda:0
        # inputs["input_ids"].device: cuda:0
        # print('inputs["lables"].device:', inputs["labels"].device)
        # print('inputs["input_features"].device:', inputs["input_features"].device)
        # print('inputs["feature_attention_mask"].device:', inputs["feature_attention_mask"].device)
        # print('inputs["attention_mask"].device:', inputs["attention_mask"].device)
        # print('inputs["input_ids"].device:', inputs["input_ids"].device)
        # 打印engine对应的model所在的device
        # engine.module.device: cuda:1
        # print('engine.module.device:', engine.module.device)
        outputs = engine(**inputs)
        loss = outputs.loss
        # 反向传播（梯度处理）
        ...
        if cur_step % gradient_accumulation_steps == 0:
            ...
            if early_stopper.check_improvement(avg_val_loss):
                # 删除之前的检查点
                if config.sql_model_save_path and engine.local_rank == 0:
                    if os.path.exists(config.sql_model_save_path):
                        shutil.rmtree(config.sql_model_save_path)
                engine.save_checkpoint(config.sql_model_save_path,tag='best',exclude_frozen_parameters=True)
                print("Model saved with val loss: {:.4f}".format(avg_val_loss))
            else:
